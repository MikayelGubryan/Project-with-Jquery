$(document).ready(function() {
    ...
    function checkWindowSizeAndAddArrows() {
        if (window.matchMedia("(max-width: 800px)").matches) {
            // Add arrow icon for specified conditions
            if (!$('.form-list').find('.arrow-icon').length) {
                $('.form-list').each(function() {
                    if ($(this).index() === 0 || $(this).hasClass('submenu-item-1') || $(this).hasClass('submenu-item-2')) {
                        $(this).append('<i class="arrow-icon fas fa-chevron-down"></i>');
                    }
                });
            }
        } else {
            // Remove arrow icons for larger screens
            $('.arrow-icon').remove();
        }
    }
