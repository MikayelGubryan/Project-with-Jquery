$(document).ready(function() {
    ...
    function checkWindowSizeAndAddArrows() {
        if (window.matchMedia("(max-width: 800px)").matches) {
            ...
            $('.form-list').not(this).removeClass('active').css('background-color', ''); // Reset color
            $('.form-list').not(this).find('.arrow-icon').remove(); // Remove arrows from other items

            $(this).toggleClass('active');
            $(this).css('background-color', $(this).hasClass('active') ? '#010114' : ''); // Toggle color

            // Toggle arrow icon
            $(this).find('.arrow-icon').remove(); // Remove existing arrow icon
            $(this).append($(this).hasClass('active') ? arrowIconUp : arrowIconDown); // Add arrow icon based on active state

            // Toggle submenu visibility
            $(this).find('.submenu-popup').toggle(); // Show/hide submenu
        } else {
            $('.arrow-icon').remove();
        }
    }
