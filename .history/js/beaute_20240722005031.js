    ...
    function checkWindowSizeAndAddArrows() {
        if (window.matchMedia("(max-width: 800px)").matches) {
            ...
            $('.appointment-form-container').on('click', '.form-list', function(event) {
                ...
                $('.form-list').not(this).find('.arrow-icon').remove(); // Ensure other arrows are removed
    
                // Toggle the clicked item
                $(this).toggleClass('active').css('background-color', $(this).hasClass('active') ? 'red' : 'whitee'); // Toggle color
                $(this).find('.arrow-icon').remove(); // Remove existing arrow icon
                $(this).append($(this).hasClass('active') ? arrowIconAbove : arrowIconDown); // Toggle arrow                      
            });
        } else {
            $('.arrow-icon').remove();
        }
