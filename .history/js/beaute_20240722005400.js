    ...
    function checkWindowSizeAndAddArrows() {
        if (window.matchMedia("(max-width: 800px)").matches) {
            ...
            $('.appointment-form-container').on('click', '.form-list', function(event) {
                ...
                // Toggle the color of the clicked item to red and reset the color of others
                $('.form-list').not(this).removeClass('active').css('background-color', '#010114'); // Reset color
                $(this).toggleClass('active').css('background-color', $(this).hasClass('active') ? 'red' : '#010114'); // Toggle color
