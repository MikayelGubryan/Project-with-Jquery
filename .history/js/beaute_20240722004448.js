    ...
    function checkWindowSizeAndAddArrows() {
        if (window.matchMedia("(max-width: 800px)").matches) {
            ...
            $('.appointment-form-container').on('click', '.form-list', function(event) {
                ...
                $(this).removeClass('inactive').addClass('active').append(arrowIconAbove);

                $('.form-list').not(this).removeClass('active').css('color', '#010114');
                $('.form-list').not(this).find('.arrow-icon').remove();
    
                // Toggle the current item
                $(this).toggleClass('active').css('color', $(this).hasClass('active') ? 'red' : '#010114'); // Toggle color
